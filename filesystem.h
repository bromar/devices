...
class Inode<Monitor>
{
  ...
  ioDevice<char*>* driver;   // null unless kind == device.  set by open
...
public:
  ...
  int read(char* buffer, size_t count)  {
    if ( kind == Inode<Monitor>::device ) return driver->read( buffer,  count); 
    // implement here read(...) on regular files here
  }
  int write(char* buffer, size_t count) {
    if ( kind == Inode<Monitor>::device ) return driver->write( buffer,  count); 
    // implement here write(...) to regular files here
  }
  int seek(off_t newOffset, int whence)  {
    if ( kind == Inode<Monitor>::device ) return driver->seek( newOffset,  whence); 
    // implement here read(...) on regular files here
  }
  int rewind() {
  ...
  int ioctl(unsigned int cmd, unsigned long arg) {
    if ( kind == Inode<Monitor>::device ) return inode->driver->ioctl( cmd,  arg); 
